### 🐢 Project C — 거북이 자동 회피 시스템

> `pose` 토픽으로 거북이의 현재 위치를 실시간으로 감지하고, 벽에 가까워지면 자동으로 방향을 전환하는 노드를 구현한다. <br>
> "센서 → 판단 → 제어" 루프를 처음으로 직접 구현하는 프로젝트로, 자율주행 로봇 제어의 가장 기본 형태다.

---

### 🎯 목표

- [ ] `pose` 토픽을 구독해 거북이 위치를 실시간으로 읽기
- [ ] 벽(경계) 근접 여부를 판단하는 로직 구현
- [ ] `cmd_vel` 토픽 발행으로 거북이 방향 전환
- [ ] 타이머 기반 주기 제어 루프 구성

---

### 📐 시스템 설계

**데이터 흐름**

```
[turtlesim] ──/turtle1/pose──▶ [wall_avoid_node]
                                      │
                              위치 판단 (x, y 경계 체크)
                                      │
                               ──/turtle1/cmd_vel──▶ [turtlesim]
```

**turtlesim 판 경계값**

| 경계 | 좌표값 |
|------|--------|
| 최솟값 (좌/하) | 약 0.5 |
| 최댓값 (우/상) | 약 10.5 |
| 안전 마진 | 1.0 권장 |

---

### 🏗️ 구현 단계

**▸ Step 1 — 패키지 및 파일 준비**
기존 `my_first_package`에 `wall_avoid.py` 파일을 추가하고 `setup.py`에 엔트리포인트를 등록한다.

```python
# setup.py entry_points에 추가
'wall_avoid = my_first_package.wall_avoid:main',
```

---

**▸ Step 2 — 노드 기본 구조**

**▸ Step 3 — 판단 및 제어 로직**
타이머 콜백에서 위치를 확인하고 `Twist` 메시지로 제어 명령을 발행한다.

```python
    SAFE_MARGIN = 1.0   # 벽에서 이 거리 이내면 회피

    def timer_callback(self):
        twist = Twist()

        # 벽 근접 판단
        too_close = (
            self.pose.x < self.SAFE_MARGIN or
            self.pose.x > 11.0 - self.SAFE_MARGIN or
            self.pose.y < self.SAFE_MARGIN or
            self.pose.y > 11.0 - self.SAFE_MARGIN
        )

        if too_close:
            # 회피: 제자리 회전
            twist.linear.x  = 0.0
            twist.angular.z = 1.5
        else:
            # 직진
            twist.linear.x  = 2.0
            twist.angular.z = 0.0

        self.pub.publish(twist)
```

---

**▸ Step 4 — 빌드 및 실행**

```bash
colcon build --packages-select my_first_package
source ~/ros2_study/install/setup.bash

# 터미널 1
ros2 run turtlesim turtlesim_node
# 터미널 2
ros2 run my_first_package wall_avoid
```

---

### 🔧 개선 아이디어

기본 구현이 완성되면 아래를 시도해본다.

- **방향 판단 개선**: 어느 벽에 가까운지에 따라 회전 방향을 다르게 설정 (왼쪽 벽 → 오른쪽으로 회전)
- **커스텀 메시지 추가**: 현재 상태(`normal` / `avoiding`)를 커스텀 메시지로 발행해 모니터링
- **속도 조절**: 벽과의 거리에 비례해 속도를 줄이는 로직 (비례 제어 기초)

---

### ✅ 완성 기준

- 거북이가 벽에 부딪히지 않고 계속 움직인다
- `rqt_graph`에서 `pose` 구독 + `cmd_vel` 발행 구조가 보인다
- `ros2 topic echo /turtle1/cmd_vel`로 제어 명령이 출력된다
